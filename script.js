...
const translations = {
    pt: {
        ...
        poolAnalyzer: {
            ...
            chart: {
                title: 'Visualização de Ranges',
                currentPrice: 'Preço Atual',
                minPrice: 'Preço Mínimo',
                maxPrice: 'Preço Máximo',
                note: '💡 Linhas coloridas = limites de cada range. Ajuste Min/Max para recalcular.'
            },
            actions: {
                copy: 'Copiar Análise',
    ...
    en: {
        ...
        poolAnalyzer: {
            ...
            chart: {
                ...
                minPrice: 'Min Price',
                maxPrice: 'Max Price',
                note: '💡 Colored lines = range limits. Adjust Min/Max to recalculate.'
            },
            actions: {
                copy: 'Copy Analysis',
...
function displayPoolResults(result) {
    ...
    // Fetch historical prices and render chart
    fetchAndRenderChart(poolData, ranges);
...
async function fetchAndRenderChart(poolData, ranges) {
    // Fetch historical prices for chart
    let historicalPrices = null;
    try {
        historicalPrices = await poolAnalyzer.fetchHistoricalPrices(poolData.quoteToken.symbol, 7);
    } catch (error) {
        console.log('Could not fetch historical data, using current price only');
    }
    ...
    // Update chart header
    const currentPriceChartEl = document.getElementById('currentPriceChart');
    if (currentPriceChartEl) {
        currentPriceChartEl.textContent = `$${poolData.priceUsd.toFixed(poolData.priceUsd < 1 ? 6 : 2)}`;
    }

    // Populate compact range prices below chart
    const shortMinCompact = document.getElementById('shortMinCompact');
    const shortMaxCompact = document.getElementById('shortMaxCompact');
    const moderateMinCompact = document.getElementById('moderateMinCompact');
    const moderateMaxCompact = document.getElementById('moderateMaxCompact');
    const longMinCompact = document.getElementById('longMinCompact');
    const longMaxCompact = document.getElementById('longMaxCompact');

    if (shortMinCompact) shortMinCompact.textContent = formatPoolPrice(ranges.short.min);
    if (shortMaxCompact) shortMaxCompact.textContent = formatPoolPrice(ranges.short.max);
    if (moderateMinCompact) moderateMinCompact.textContent = formatPoolPrice(ranges.moderate.min);
    if (moderateMaxCompact) moderateMaxCompact.textContent = formatPoolPrice(ranges.moderate.max);
    if (longMinCompact) longMinCompact.textContent = formatPoolPrice(ranges.long.min);
    if (longMaxCompact) longMaxCompact.textContent = formatPoolPrice(ranges.long.max);

    // Analyze position and show alert (compact version above chart)
    const positionAnalysis = poolAnalyzer.analyzePricePosition(poolData.priceUsd, ranges);
    const alertBox = document.getElementById('rangeAlert');
    if (alertBox) {
        const alertLevels = {
            safe: { bg: 'rgba(16, 185, 129, 0.1)', border: '#10b981' },
            info: { bg: 'rgba(168, 85, 247, 0.1)', border: '#a855f7' },
            caution: { bg: 'rgba(245, 158, 11, 0.1)', border: '#f59e0b' },
            warning: { bg: 'rgba(251, 146, 60, 0.1)', border: '#fb923c' },
            danger: { bg: 'rgba(239, 68, 68, 0.1)', border: '#ef4444' }
        };

        alertBox.style.display = 'flex';
        alertBox.style.background = alertLevels[positionAnalysis.level].bg;
        alertBox.style.borderColor = alertLevels[positionAnalysis.level].border;

        const alertIcon = alertBox.querySelector('.alert-icon');
        if (alertIcon) alertIcon.textContent = positionAnalysis.icon;

        const alertMessage = alertBox.querySelector('.alert-message');
        if (alertMessage) alertMessage.textContent = positionAnalysis.message[currentLang];
    }
    ...
    if (minPriceInput) minPriceInput.value = ranges.long.min.toFixed(6);
    if (maxPriceInput) maxPriceInput.value = ranges.long.max.toFixed(6);
}
...
/**
 * Render simplified price chart with colored zones
 */
function renderRangeChart(historicalData, currentPrice, ranges, baseToken, quoteToken) {
    ...
    if (window.rangeChartInstance) {
        window.rangeChartInstance.destroy();
    }

    // Prepare data - show last 3-4 days only
    let labels, priceData;

    if (historicalData && historicalData.length > 0) {
        // Show only last 4 days
        const recentData = historicalData.slice(-4);
        labels = recentData.map(d => d.date.toLocaleDateString(currentLang === 'pt' ? 'pt-BR' : 'en-US', { month: 'short', day: 'numeric' }));
        priceData = recentData.map(d => d.price);

        // Add current price
        labels.push(currentLang === 'pt' ? 'Agora' : 'Now');
        priceData.push(currentPrice);
    } else {
        labels = [currentLang === 'pt' ? 'Agora' : 'Now'];
    ...
    // Chart configuration - simplified with colored zones
    const config = {
        type: 'line',
        data: {
            labels: labels,
    ...
    // Create chart
    window.rangeChartInstance = new Chart(context, config);

    // Store pool data for controls
    window.currentPoolData = { currentPrice, ranges, baseToken, quoteToken, historicalData };
}

/**
 * Setup price control buttons
 */
function setupPriceControls() {
    ...
    controlBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const step = currentValue < 1 ? 0.0001 : 0.01;

            if (btn.classList.contains('plus')) {
                input.value = (currentValue + step).toFixed(6);
            } else {
                input.value = Math.max(0, currentValue - step).toFixed(6);
            }

            // Recalculate ranges with new values
            updateRangesFromControls();
        });
    });
}

/**
 * Update ranges when price controls change
 */
function updateRangesFromControls() {
    ...
    // Recalculate ranges based on new center price
    const newRanges = poolAnalyzer.calculateRanges(centerPrice, 0);

    // Update range displays in range cards
    const shortMinEl = document.getElementById('shortMin');
    const shortMaxEl = document.getElementById('shortMax');
    const shortPercentEl = document.getElementById('shortPercent');
    if (shortMinEl) shortMinEl.textContent = formatPoolPrice(newRanges.short.min);
    if (shortMaxEl) shortMaxEl.textContent = formatPoolPrice(newRanges.short.max);
    if (shortPercentEl) shortPercentEl.textContent = `±${newRanges.short.percent.toFixed(1)}%`;

    const modMinEl = document.getElementById('moderateMin');
    const modMaxEl = document.getElementById('moderateMax');
    const modPercentEl = document.getElementById('moderatePercent');
    if (modMinEl) modMinEl.textContent = formatPoolPrice(newRanges.moderate.min);
    if (modMaxEl) modMaxEl.textContent = formatPoolPrice(newRanges.moderate.max);
    if (modPercentEl) modPercentEl.textContent = `±${newRanges.moderate.percent.toFixed(1)}%`;

    const longMinEl = document.getElementById('longMin');
    const longMaxEl = document.getElementById('longMax');
    const longPercentEl = document.getElementById('longPercent');
    if (longMinEl) longMinEl.textContent = formatPoolPrice(newRanges.long.min);
    if (longMaxEl) longMaxEl.textContent = formatPoolPrice(newRanges.long.max);
    if (longPercentEl) longPercentEl.textContent = `±${newRanges.long.percent.toFixed(1)}%`;

    // Update compact range prices below chart
    const shortMinCompact = document.getElementById('shortMinCompact');
    const shortMaxCompact = document.getElementById('shortMaxCompact');
    const moderateMinCompact = document.getElementById('moderateMinCompact');
    const moderateMaxCompact = document.getElementById('moderateMaxCompact');
    ...
    if (shortMinCompact) shortMinCompact.textContent = formatPoolPrice(newRanges.short.min);
    if (shortMaxCompact) shortMaxCompact.textContent = formatPoolPrice(newRanges.short.max);
    if (moderateMinCompact) moderateMinCompact.textContent = formatPoolPrice(newRanges.moderate.min);
    if (moderateMaxCompact) moderateMaxCompact.textContent = formatPoolPrice(newRanges.moderate.max);
    if (longMinCompact) longMinCompact.textContent = formatPoolPrice(newRanges.long.min);
    if (longMaxCompact) longMaxCompact.textContent = formatPoolPrice(newRanges.long.max);

    // Update range observations with new ranges
    const strategy = poolAnalyzer.detectPoolStrategy(
        window.currentPoolData.baseToken,
        window.currentPoolData.quoteToken
    );
    const rangeObs = poolAnalyzer.generateRangeObservations(
        strategy,
        newRanges,
        window.currentPoolData.currentPrice,
        currentLang
    );
    ...
    // Re-render chart with new ranges
    renderRangeChart(
        window.currentPoolData.historicalData,
        window.currentPoolData.currentPrice,
        newRanges,
        window.currentPoolData.baseToken,
        window.currentPoolData.quoteToken
