    ...
    pt: {
        ...
        poolAnalyzer: {
            ...
            chart: {
                ...
                note: '💡 Use os controles acima para ajustar os preços min/max e recalcular os ranges dinamicamente. O gráfico mostra o histórico de 7 dias do preço do token.'
            },
            actions: {
                copy: 'Copiar Análise',
    ...
    en: {
        ...
        poolAnalyzer: {
            ...
            chart: {
                ...
                minPrice: 'Min Price',
                maxPrice: 'Max Price',
                note: '💡 Use the controls above to adjust min/max prices and recalculate ranges dynamically. The chart shows the token\'s 7-day price history.'
            },
            actions: {
                copy: 'Copy Analysis',
...
async function fetchAndRenderChart(poolData, ranges) {
    ...
    const pricePairChartEl = document.getElementById('pricePairChart');
    if (pricePairChartEl) {
        pricePairChartEl.textContent = `${poolData.quoteToken.symbol} per ${poolData.baseToken.symbol}`;
    }

    // Analyze position and show alert
    ...
    if (minPriceLabel) minPriceLabel.textContent = `${poolData.quoteToken.symbol} per ${poolData.baseToken.symbol}`;
    if (maxPriceLabel) maxPriceLabel.textContent = `${poolData.quoteToken.symbol} per ${poolData.baseToken.symbol}`;
}
...
/**
 * Render price chart with range indicators
 */
function renderRangeChart(historicalData, currentPrice, ranges, baseToken, quoteToken) {
    ...
    const labels = historicalData
        ? historicalData.map(d => d.date.toLocaleDateString(currentLang === 'pt' ? 'pt-BR' : 'en-US', { month: 'short', day: 'numeric' }))
        : ['Now'];
    ...
    // Add current price at end if historical data exists
                ...
                tooltip: {
                    ...
                    callbacks: {
                        label: function(context) {
                            ...
                            return `Price: $${price.toFixed(price < 1 ? 6 : 2)}`;
                        }
                    }
                },
                annotation: {
                    annotations: {
                        ...
                        longMax: {
                            ...
                            borderDash: [5, 5],
                            label: {
                                content: `🟢 Long Max: $${ranges.long.max.toFixed(4)}`,
                                enabled: true,
                                position: 'end',
                        ...
                        moderateMax: {
                            ...
                            borderDash: [5, 5],
                            label: {
                                content: `🟡 Mod Max: $${ranges.moderate.max.toFixed(4)}`,
                                enabled: true,
                                position: 'end',
                        ...
                        shortMax: {
                            ...
                            borderWidth: 2,
                            label: {
                                content: `🔴 Short Max: $${ranges.short.max.toFixed(4)}`,
                                enabled: true,
                                position: 'end',
                        ...
                        shortMin: {
                            ...
                            borderWidth: 2,
                            label: {
                                content: `🔴 Short Min: $${ranges.short.min.toFixed(4)}`,
                                enabled: true,
                                position: 'start',
                                backgroundColor: 'rgba(239, 68, 68, 0.8)',
                                color: '#fff',
                        ...
                        moderateMin: {
                            ...
                            borderDash: [5, 5],
                            label: {
                                content: `🟡 Mod Min: $${ranges.moderate.min.toFixed(4)}`,
                                enabled: true,
                                position: 'start',
                                backgroundColor: 'rgba(245, 158, 11, 0.8)',
                                color: '#fff',
                        ...
                        longMin: {
                            ...
                            borderDash: [5, 5],
                            label: {
                                content: `🟢 Long Min: $${ranges.long.min.toFixed(4)}`,
                                enabled: true,
                                position: 'start',
                                backgroundColor: 'rgba(16, 185, 129, 0.8)',
                                color: '#fff',
                        ...
                        currentPrice: {
                            ...
                            borderWidth: 3,
                            label: {
                                content: `Current: $${currentPrice.toFixed(4)}`,
                                enabled: true,
                                position: 'center',
...
function updateRangesFromControls() {
    ...
    document.getElementById('shortMin').textContent = formatPoolPrice(newRanges.short.min);
    document.getElementById('shortMax').textContent = formatPoolPrice(newRanges.short.max);
    document.getElementById('shortPercent').textContent = `±${newRanges.short.percent.toFixed(1)}%`;

    document.getElementById('moderateMin').textContent = formatPoolPrice(newRanges.moderate.min);
    document.getElementById('moderateMax').textContent = formatPoolPrice(newRanges.moderate.max);
    document.getElementById('moderatePercent').textContent = `±${newRanges.moderate.percent.toFixed(1)}%`;

    document.getElementById('longMin').textContent = formatPoolPrice(newRanges.long.min);
    document.getElementById('longMax').textContent = formatPoolPrice(newRanges.long.max);
    document.getElementById('longPercent').textContent = `±${newRanges.long.percent.toFixed(1)}%`;
    ...
    document.getElementById('shortObservation').textContent = rangeObs.short;
    document.getElementById('moderateObservation').textContent = rangeObs.moderate;
    document.getElementById('longObservation').textContent = rangeObs.long;
